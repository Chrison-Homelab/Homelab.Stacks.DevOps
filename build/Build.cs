...
// DevOps stack pipeline — Fallout build.
//
...
class Build : FalloutBuild
{
    ...
    const string StackRepo = "Chrison-Homelab/Homelab.Stacks.DevOps";
    ...
    Target Bundle => _ => _
        ...
        .Executes(() =>
        {
            ...
            var archive = DistDirectory / $"devops-{version}.tar.gz";
    ...
    {
        // ⚠ HAND-MAINTAINED, and a new asset directory that is not added here is silently
        // left out of the bundle rather than failing the build. Ported as-is from SmartHome
        // rather than made clever, but it is the one part of this harness that rots.
        // Cross-check against `assets:` in the shapes when adding a member.
        var candidates = new[]
        {
    ...
    Target Release => _ => _
        ...
        .Executes(() =>
        {
            ...
            Gh($"release create {tag} {DistDirectory / $"devops-{version}.tar.gz"} {DistDirectory / "MANIFEST.md"} " +
               $"--repo {StackRepo} --title \"DevOps {tag}\" --generate-notes --target {Git("rev-parse HEAD").Trim()}");
